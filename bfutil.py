...
class BFUtil:
    ...
    def convert(self, i_brew):
        ...
        if "batchRating" in i_brew:
            bf["tasteRating"] = 10 * 1.25 * i_brew["batchRating"]  # iBrewMaster goes from 0-8, BF from 0-100
    ...
    def add_extra(bf, extra):
        ...
        if name in BFUtil.EXTRAS_MAP:
            ...
            # Changed additions/extras to just use the units specified in the map file and no conversions
            bf_extra["amount"] = extra["beAmount"]
            bf_extra["time"] = get_value(extra, "beTime", 0)
